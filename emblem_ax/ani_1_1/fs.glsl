...
float anil1(vec2 uv){
    float liTime=iTime-1.;
    vec2 tuv = uv*MD(PI / 4.);
    float a = atan(tuv.x,tuv.y);
    float r = length(tuv);
    tuv = vec2(a/TWO_PI,r);
    float d=smoothstep(-PI+liTime/.5,-PI+liTime/.5+2./iResolution.y,tuv.x);
    return 1.-d;
}

float anil2(vec2 uv){
    float liTime=iTime-0.5;
    //return 1.- shape(uv*4., 4, 0.980, 0.20+iTime/3.);
    float d=smoothstep(0.+liTime/1.-2./iResolution.y,0.+liTime/1.,abs(uv.x));
    return 1.-d;
}

float anil3(){
    float liTime=iTime-1.;
    return smoothstep(0.,.5,liTime/1.);
}
...
float anil5(vec2 uv){
    float liTime=iTime+.25;
    vec2 tuv = uv*MD(PI / 4.);
    float a = atan(tuv.x,-tuv.y);
    float r = length(tuv);
    tuv = vec2(a/TWO_PI,r);
    float d=smoothstep(-PI+liTime/.4,-PI+liTime/.4+2./iResolution.y,abs(tuv.x));
    return 1.-d;
}
...
float anil6(vec2 uv){
    ...
    float r = length(tuv);
    tuv = vec2(a/TWO_PI,r);
    tuv.x=(1.)*floor((tuv.x)/(1.))+.8;
    float d=smoothstep(-PI+liTime/.5,-PI+liTime/.5+2./iResolution.y,(tuv.x));
    return 1.-d;
}

float anil61(vec2 uv){
    float liTime=iTime-1.;
    //iTime=iMouse.x/iResolution.y*iTime;
    vec2 tuv = uv*MD(PI / 4.);
    float a = atan(tuv.x,tuv.y);
    float r = length(tuv);
    tuv = vec2(a/TWO_PI,r);
    float tuvx=(1.)*floor((tuv.x)/(1.));
    float d=1.-smoothstep(-PI+liTime/.5,-PI+liTime/.5+2./iResolution.y,(tuvx));
    d=d*smoothstep(-PI+liTime/.5,-PI+liTime/.5+2./iResolution.y,(tuvx+1.));
    return d*smoothstep(0.,2.065/4.,mod(liTime,2.065/4.));
}

float anil_w(float liTime){
    if(liTime>6.){
		float liTimex=mod(liTime,8.);
		if(liTimex>6.){
    	return smoothstep(0.,2.,mod(liTimex,2.));}
    	return 0.;}
    return 0.;
}

float anil_w2(float liTime){
    if(liTime>6.){
    	return smoothstep(0.,2.,mod(liTime,2.));}
...
void mi_o(out vec4 fragColor, in vec2 fragCoord,float pxx) {
    ...
    global_zoom = 4.;
    tuv = uv*global_zoom;
    d = max(d, col2 * shape(tuv, 4, 0.750, 0.4)*anil1(uv));
    tuv = uv;
    ...
    tres*=tz;
    tres*=(1.-1.*pxx);
    vec4 tcol = texture2D(iChannel0,tres)*dx;
...
void mi_o_dx(out vec4 fragColor, in vec2 fragCoord,float pxx) {
    vec2 res = iResolution.xy / iResolution.y;
    vec2 uv = (fragCoord.xy) / iResolution.y - res / 2.0;
    uv*=1.-1.*pxx;
    //uv *= MD(iTime);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    mi_o(fragColor,fragCoord,anil_w(0.));
    if(anil_w2(iTime)>0.){
        vec4 tcb=vec4(0.);
        mi_o_dx(tcb,fragCoord,.85*anil_w2(iTime));
    	vec4 dxf=tcb;
    	dxf*=1.-2.*anil_w2(iTime);
    	dxf=max(fragColor,dxf);
    	tcb=vec4(0.);
    	if(anil_w(iTime)>0.){
